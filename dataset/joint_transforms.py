...
class RandomCrop_stacked(object):
    '''
    Take a random crop from the image.
    '''
    def __init__(self, size, ignore_index=0, nopad=True):
    ...
    def __call__(self, img, coords, mask, centroid=None):
        assert img.size == mask.size
        coords = to_pil_image(coords)
        w, h = img.size
        # ASSUME H, W
        th, tw = self.size
        if w == tw and h == th:
        ...
        if self.nopad:
            ...
            if pad_h or pad_w:
                img = ImageOps.expand(img, border=border, fill=self.pad_color)
                mask = ImageOps.expand(mask, border=border, fill=self.ignore_index)
                coords = ImageOps.expand(coords, border=border, fill=self.ignore_index)
                w, h = img.size
        ...
        coords = coords.crop((x1, y1, x1 + tw, y1 + th))
        # coords = crop(coords, x1, y1, x1 + tw, y1 + th)
        mask = mask.crop((x1, y1, x1 + tw, y1 + th))
        return img, coords, mask
