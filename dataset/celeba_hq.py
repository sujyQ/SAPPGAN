'''
    This code is from https://github.com/dolphin0104/MSPL-GAN
'''
...
import random
from dataset import joint_transforms, data_utils
from torch.utils.data import DataLoader


class CelebA_HQ(data.Dataset):
    """Dataset class for the CelebA_HQ dataset."""
    def __init__(self, data_dir, n_classes=19, imsize=128, mode='train'):        
        ...
        if mode == 'train':
            self.trans = joint_transforms.Compose([
                ...
                joint_transforms.Resize(self.imsize),
        ...
        self.label_trasform = transforms.ToTensor()
    

    def _set_filelist(self):
        img_dir = os.path.join(self.data_dir, '{}_img'.format(self.mode))
        if self.n_classes == 19 :
            mask_dir = os.path.join(self.data_dir, '{}_label'.format(self.mode))
    ...
    def __getitem__(self, idx):
        ...
        gt_segmap = self.label_trasform(gt_segmap) 
        
        return gt_img, gt_segmap, filename
